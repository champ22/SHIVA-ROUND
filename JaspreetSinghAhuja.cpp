...
{
    cout<<"Enter number of Faculty Appointments\n";
    cin>>fac_proc;
    
    cout<<"Enter number of student Appointments\n";
    cin>>stu_proc;
    
    
    nr_proc= stu_proc + fac_proc;
    cout<<"The total number of Appointments="<<nr_proc<<"\n";
    remain_processes = nr_proc;
    cout<<"Enter the Burst and Arrival times in order :\n\n";
    
    
    cout<<"\nENTER FACULTY DETAILS\n\n";
	for(int i=0;i<fac_proc;i++){
        int bt, at;
        cout<<"Requested (BURST) Time for Faculty(" << i+1 <<"): ";
        cin>>bt;
        cout<<"Arrival Time for Faculty(" << i+1 <<"): ";
        cin>>at;
        processes.push_back(Process(bt, at));
    }
    
    cout<<"\nENTER STUDENT DETAILS\n\n";
    for(int i=0;i<stu_proc;i++){
        int bt, at;
        cout<<"Requested (BURST) Time by Student(" << i+1 <<"): ";
        cin>>bt;
        cout<<"Arrival Time for Student(" << i+1 <<"): ";
        cin>>at;
        processes.push_back(Process(bt, at));
	}
}
...
void RoundRobin::displayTime()
{
    ...
    cout<<"\n\n--------------------------------------\n";
    cout<<"Appointment(Process)\t|TurnAround Time|Waiting Time\n";
    cout<<"--------+---------------+-------------\n";
    for(int i=0;i<nr_proc;i++)
    {
    ...
    cout<<"THE SCHEDULE HAS BEEN GENERATED USING ROUND ROBIN TECHNIQUE \n";
}
...
void RoundRobin::computeRR(){
    ...
    int i;
    int time_quantum;
    cout<<"\nEnter the maximum ttime of Appointment(QUANTUM):\n";
    cin>>time_quantum;
